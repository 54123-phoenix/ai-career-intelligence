          ...
          <div key={i} className="flex gap-4 pb-4">
            ...
            <div className="flex-1 pb-2">
              <div className="flex items-center gap-2 mb-1">
                <span className={`rounded px-2 py-0.5 text-xs font-medium ${actorBadges[ev.actor] || ""}`}>
                  {ev.actor}
                </span>
                <span className="text-sm font-medium text-gray-900">{ev.action_label}</span>
                {ev.score != null && (
                  <span className="text-xs font-mono text-gray-500">score: {ev.score.toFixed(2)}</span>
                )}
              </div>
